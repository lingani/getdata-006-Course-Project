...
merged_train_and_test_set$subjectID= c(subject_train$subjectID, subject_test$subjectID)
...
#names(selected_data)<-selected_features$featuresLabel
selected_data<-cbind( subjectID=merged_train_and_test_set$subjectID, experiment=merged_train_and_test_set$experiment, selected_data)
...
names(train_activity_labels)<-c("activityID", "activity")
...
names(test_activity_labels)<-c("activityID", "activity")
...
selected_data<-cbind(activity=merged_activity_labels$activity, selected_data)
...
tidy_data<-ddply(data, .(activity, subjectID), colwise(mean))
